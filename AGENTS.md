...
- A fixed range is a run of calendar days, not a rolling span of milliseconds. `calendarWindow` opens it at local midnight on the first of those days, because a day is the coarsest thing the calendar surfaces can draw and a window they cannot draw is one they cannot agree with. ALL is the whole ledger and starts on its oldest session.
- A chart's bucket ceiling is a resolution budget, never a window. `series` in `portal/usage-model.js` takes that ceiling and picks the finest unit -- day, then week, then month -- that spans the whole window inside it, so a total drawn from the series is the period's total. Capping the bucket count instead silently drops every session older than the cap. A bucket carries the unit it was built at, and every label naming a cadence names that unit.
...
- opencode keeps every session in one SQLite database, so `FoundSession.transcriptPath` is that database and the session id is the key, not a fallback. A session's usage folds in its `parent_id` descendants. Reconciliation candidates are roots only, and an id from anywhere in a tree resolves to that tree's root, because opencode names descendant sessions in its own events.
- An opencode session stays writable, so its usage and the fingerprint pinning it come out of one `oneRead` transaction on one open. One open is not enough: outside a transaction SQLite gives every statement its own snapshot, and a message row landing between two of them would be fingerprinted without being billed, which the next sync reads as no change at all.
...
- Copilot usage comes from the persisted `session.shutdown.modelMetrics` aggregate; incomplete sessions without shutdown are not capture candidates. Discovery decides that from a bounded tail rather than a full parse, since the aggregate is the last record written: a shutdown outside the tail is not a completed session.
